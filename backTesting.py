...
class backTesting :
    ...
    def execute(self) :
        ...
        for idx, row in df.iterrows() :
            # 매수 신호 확인
            # 목표가에 달성한 경우 목표가에 매수해 다음날 시가에 매도한 것으로 판단 + 상승장 조건 추가
            self.buy_signal = np.where((row['targetPrice'] <= row['high']) & row['bull'], True, False)
            ...
            self.ror = row['close'] / row['targetPrice'] - self.fee if self.buy_signal else 1
